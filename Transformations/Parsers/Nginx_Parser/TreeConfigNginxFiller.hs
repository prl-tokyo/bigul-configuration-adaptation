...
instruction ::: Instruction
    ...
    / "#ID" [a-z 0-9]+ { ("ID",$1) }
    / [0-9 a-z A-Z _ / \ ' : = ! ~ ^ { } \[ \] \- \. \* $ "]+ ';' { removeFirstSpace (break (==' ') $1) }
block ::: TreeFile 
    = "location" [a-z A-Z 0-9 / \- \_]* "{" directive* "}" { let ds = catMaybes $2 in TreeFile "location" ([("location_path",fst (break (==' ') $1))]++lefts ds) (rights ds) }
    / [a-z /]+ "{" directive* "}" { let ds = catMaybes $2 in TreeFile (fst (break (==' ') $1)) (lefts ds) (rights ds) }
comment ::: ()
    = "#" [^\n]* { () }
|]
--}}}
...
parseTreeNginx file = return . parseString configFileNginx "<stdin>" =<< readFile file
---}}}
